#!/bin/bash
#
# Installation script for webOS SDK
...
# through the "Current" symlink (./Current/install-sdk-macos.sh). Without it,
...
if [ "$EUID" -ne 0 ]; then
    log_error "Please run with sudo: sudo ./install-sdk-macos.sh"
    exit 1
fi
...
echo ""
echo "=========================================="
echo "  webOS SDK Installation for macOS"
echo "=========================================="
echo ""
...
echo "  3) /opt/homebrew/PalmSDK (Homebrew-adjacent location)"
echo "  4) Custom location"
echo ""
read -p "Enter choice [1-4]: " sdk_choice
...
case $sdk_choice in
    ...
    2)
        ...
        ;;
    3)
        SDK_INSTALL_DIR="/opt/homebrew/PalmSDK"
        ;;
    4)
...
echo "  2) /opt/homebrew/bin (Homebrew standard)"
echo "  3) Same as SDK location (${SDK_INSTALL_DIR}/bin symlinked to PATH manually)"
echo "  4) Custom location"
echo ""
read -p "Enter choice [1-4]: " bin_choice
...
chown -R root:wheel "$SDK_INSTALL_DIR/$SDK_VERSION"
...
if [ "$CREATE_SYMLINKS" = true ]; then
    if echo "$PATH" | grep -q "$BIN_INSTALL_DIR"; then
        ...
        if [ -n "$SHELL_RC" ]; then
            ...
            echo ""
            echo "  echo 'export PATH=\"\$PATH:$BIN_INSTALL_DIR\"' >> $SHELL_RC"
    ...
    log_info "To add SDK commands to your PATH, run:"
    echo ""
    SHELL_RC=""
    if [ -n "$ZSH_VERSION" ]; then
        SHELL_RC="$HOME/.zshrc"
    ...
    if [ -n "$SHELL_RC" ]; then
        echo "  echo 'export PATH=\"\$PATH:$SDK_INSTALL_DIR/$SDK_VERSION/bin\"' >> $SHELL_RC"
        echo "  source $SHELL_RC"
    else
